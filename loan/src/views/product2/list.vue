  ...
  <div>
    <el-form :inline="true" :model="form" ref="form" size="small" label-width="100px" >
      <el-form-item label="关键词：" prop="productName">
        <el-input v-model="form.productName" placeholder="请输入关键词"></el-input>
      </el-form-item>
      <el-form-item label="所属频道：" prop="channelCode">
        <el-select
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="props">
          <el-tag type="info" size="small" style="margin-right:5px;" v-for="(item, index) in props.row.channelList" :key="index">
            {{item.channelCode}} - {{item.productPriority}}
          </el-tag>
        </template>
      </el-table-column>
      <el-table-column
...
export default {
  ...
  methods: {
    ...
    upDateList (row, updateObj = {}) {
      ...
      productApi['PRODUCT_STATUS'](Object.assign({}, updateObj)).then(response => {
        const result = response.data
        if (result.code === 200) {
          this.getSyncList()
        } else {
    ...
    getSyncList (option = {}) {
      ...
      productApi['PRODUCT_LIST'](Object.assign(
        this.getParam(),
        option
      )).then(response => {
        const result = response.data
