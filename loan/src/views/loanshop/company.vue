<template>
  <div>
    ...
    <el-row style="margin:0px 0 10px;">
      <el-col :span="24" >
        <el-button size="small" type="primary" icon="el-icon-edit-outline" @click.native='onAdd'>添加新渠道</el-button>
      </el-col>
    </el-row>
    <el-table
      class="tableList"
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button @click="handleManage(scope.row)" size="mini" icon="el-icon-view" type="danger" plain>业务管理</el-button>
...
import dialogChannel from './component/dialogCompany'
...
export default {
  ...
  methods: {
    ...
    // 业务管理
    handleManage (row) {
      this.$router.push({path: 'canalurl', query: {canalId: row.canalId}})
    },
    // 对接销售
    getSalesOption (query = '') {
      // if (query !== '') {
      companyApi['BUSINESS_CANAL_SALES']().then(response => {
    ...
    setIsOpen (isEnable, row) {
      this.upDateList(row, {isEnable, canalId: row.canalId})
    },
    // 更新接口数据
    upDateList (row, updateObj = {}) {
      // 需要传多个额外参数的时候需要用到 assign
      companyApi['COMPANY_STATUS'](Object.assign({}, updateObj)).then(response => {
        const result = response.data
        if (result.code === 200) {
          this.getSyncList()
        } else {
    ...
    getSyncList (option = {}) {
      ...
      companyApi['COMPANY_LSITS'](Object.assign(
        this.getParam(),
        option
      )).then(response => {
        const result = response.data
