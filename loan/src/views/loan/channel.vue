<template>
  <div>
    <el-form :inline="true" :model="form" ref="form" size="small" label-width="100px" >
      <el-form-item label="关键词：" prop="channelName">
        <el-input v-model="form.channelName" placeholder="请输入关键词"></el-input>
      </el-form-item>
    ...
    <el-row style="margin:0px 0 10px;">
      <el-col :span="24" >
        <el-button size="small" type="primary" icon="el-icon-edit-outline" @click.native='onAdd'>添加频道</el-button>
      </el-col>
    </el-row>
    <el-table
      class="tableList"
...
import dialogChannel from './component/dialogChannel'
...
export default {
  ...
  methods: {
    ...
    setIsOpen (isEnable, row) {
      this.upDateList(row, {isEnable, channelId: row.id})
    },
    // 更新接口数据
    upDateList (row, updateObj = {}) {
      // 需要传多个额外参数的时候需要用到 assign
      productApi['CHANNEL_STATUS'](Object.assign({}, updateObj)).then(response => {
    ...
    getSyncList (option = {}) {
      ...
      productApi['CHANNEL_LIST'](Object.assign(
        this.getParam(),
        option
      )).then(response => {
        const result = response.data
