<template>
  <div class="page">
    ...
    <section class="pagehead">
      <h2 class="log-title">{{myValue.title}}</h2>
      <h4 class="log-sub-title">{{myValue.createTime}}</h4>
      <div class="description">
        <p class="desccon">{{myValue.content}}</p>
      </div>
    </section>
  </div>
</template>
...
export default {
  ...
  created () {
    ...
    this.myValue = JSON.parse(localStorage.getItem('item'))
    if (this.myValue.id.length === 0) {
      this.$router.go(-1)
    }
  },
  // computed: {
  //   newscontent () {
  //     return JSON.parse(localStorage.getItem('item')).content
  //   }
  // },
  methods: {
    onBlur () {
