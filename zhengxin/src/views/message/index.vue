<template>
  <div class="page">
    ...
    <section v-else>
      ...
      <p class="empty">
        <img src="../../../static/empty.png" style="display:block;">
        空空如也~
        </p>
    </section>
...
export default {
  ...
  methods: {
    ...
    toDetail (item) {
      window.location.href = '/authentication/message/'+item.id
      // this.$router.push({path: `/authentication/message/${item.id}`})
  ...
  mounted () {
    ...
    .then((response) => {
      ...
      if (res.code === 200) {
        ...
        this.message = response.data.datas
